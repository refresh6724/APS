...
public class Main_JO_1137_책복사하기 { // 제출일 2021-07-06 23:59
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < tc; i++) {
			...
			m = Integer.parseInt(st.nextToken()); // 책의 권수 500 이하
			k = Integer.parseInt(st.nextToken()); // 서기공의 수 m 이하
			max = 0;
			page = new int[m];
			st = new StringTokenizer(br.readLine());
			for (int j = 0; j < m; j++) {
				page[j] = Integer.parseInt(st.nextToken()); // 페이지 수 각 1억 이하
				max = Math.max(max, page[j]);
			}
			// 가장 많은 페이지를 맡은 서기공의 페이지가 최소가 되도록 책을 배분

			long low = 0;
			long high = 50000000000L; // 500억= 500 명 * 1억
