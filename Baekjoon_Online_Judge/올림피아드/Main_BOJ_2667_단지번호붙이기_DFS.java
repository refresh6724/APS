...
public class Main { // 제출일 2019-08-28 13:09
	...
	private static void paint(int i, int j) {
		map[i][j] = num;
		if(i-1>=0 && input[i-1][j] == 1 &&map[i-1][j] == 0) paint(i-1,j);
		if(i+1<N && input[i+1][j] == 1 && map[i+1][j] == 0) paint(i+1,j);
		if(j-1>=0 && input[i][j-1] == 1 && map[i][j-1] == 0) paint(i,j-1);
		if(j+1<N && input[i][j+1] == 1 && map[i][j+1] == 0) paint(i,j+1);		
	}
}
